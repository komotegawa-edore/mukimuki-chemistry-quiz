  ...
  useEffect(() => {
    ...
    if (errorParam) {
      const errorMessages: Record<string, string> = {
        ...
        rate_limit: '短時間に複数回の試行が行われたため、一時的にブロックされています。',
      }
      ...
      <div className="max-w-md w-full">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <div className="mb-8 text-center">
            ...
            <h1 className="text-2xl font-bold text-gray-800">RoopyNote</h1>
            <p className="text-gray-500 text-sm mt-1">受験生の学習記録アプリ</p>
          </div>
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <Link href="/note/signup" className="text-amber-500 hover:text-amber-600 font-semibold ml-2">
              新規登録
            </Link>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          <div className="bg-white/80 backdrop-blur rounded-xl p-3 text-center">
            <BookOpen className="w-6 h-6 text-amber-500 mx-auto mb-1" />
            <p className="text-xs text-gray-600">学習記録</p>
          </div>
          <div className="bg-white/80 backdrop-blur rounded-xl p-3 text-center">
            <Target className="w-6 h-6 text-orange-500 mx-auto mb-1" />
            <p className="text-xs text-gray-600">目標管理</p>
          </div>
          <div className="bg-white/80 backdrop-blur rounded-xl p-3 text-center">
            <PenLine className="w-6 h-6 text-amber-600 mx-auto mb-1" />
            <p className="text-xs text-gray-600">つぶやき</p>
          </div>
        </div>
      </div>
    </div>
...
export default function NoteLoginPage() {
  return <LoginForm />
}
