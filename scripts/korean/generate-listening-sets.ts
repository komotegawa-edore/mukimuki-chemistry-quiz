 ...
 *
 * 使い方:
 *   npx tsx scripts/korean/generate-listening-sets.ts
 *
 * 生成内容:
 *   - 10セット × 3問 = 30問
 *   - 各セットは2-3行の韓国語スクリプト
...
async function generateSet(setNumber: number): Promise<GeneratedSet> {
  const situation = SITUATIONS[(setNumber - 1) % SITUATIONS.length]

  console.log(`\n📝 セット${setNumber}を生成中... (${situation})`)

  const prompt = `あなたは韓国語教師です。リスニング練習用の韓国語スクリプトと問題を生成してください。
...
例:
{
  "korean_script": "손님 여러분께 안내 말씀드립니다. 서울행 KTX 열차는 3번 승강장에서 오전 9시 45분에 출발합니다. 승차권을 미리 준비해 주시기 바랍니다.",
  "japanese_translation": "お客様にご案内いたします。ソウル行きKTX列車は3番ホームから午前9時45分に出発します。乗車券をあらかじめご準備ください。",
  "romanization": "sonnim yeoreobunkkae annnae malsseumdeurimnida...",
  "questions": [
    {
      "question_text": "列車は何時に出発しますか？",
      "choice_a": "9:15",
      "choice_b": "9:45",
      "choice_c": "10:15",
      "choice_d": "10:45",
      "correct_answer": "B",
      "explanation": "「오전 9시 45분에 출발합니다」と言っているため"
    }
  ]
}`
...
async function main() {
  console.log('🇰🇷 韓国語リスニングセット生成スクリプト')
  console.log('==========================================\n')
  ...
  for (let setNum = 1; setNum <= CONFIG.setsToGenerate; setNum++) {
    try {
      // スクリプトと問題を生成
      const setData = await generateSet(setNum)
      const setId = `KLS${String(setNum).padStart(3, '0')}`
      ...
      const { error: setError } = await supabase
        ...
        .upsert({
          ...
          audio_url: audioUrl,
          category: SITUATIONS[(setNum - 1) % SITUATIONS.length],
          is_published: true,
        })
